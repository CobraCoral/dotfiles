...
class PlayerSegment(Segment):
	def __call__(self, format='{state_symbol} {artist} - {title} ({total})', state_symbols=STATE_SYMBOLS, **kwargs):
		def getContents():
			tags = re.findall('\{(.*?)\}', format)
			rc = re.compile('([a-z]+)(:(.+?)([0-9]+))?')
			for tag in tags:
				match = re.match(rc, tag)
				if match:
					groups = match.groups()
					value = ('{'+'s[{}]'.format(groups[0])+'}').format(s=stats)
					if groups[1]:
						width = int(groups[3])
						if width < len(value):
							value = value[:width]
						value = '{:{align}{width}}'.format(value, align=groups[2], width=groups[3])
				else:
					value = None
					return
				stats[groups[0]] = value
		stats = {
			'state': 'fallback',
			'album': None,
			'artist': None,
